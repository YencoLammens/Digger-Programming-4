...
PlotStatus Plot(const char* label, const PlotConfig& conf) {
    ...
    const ImRect frame_bb(
        window->DC.CursorPos,
        ImVec2(window->DC.CursorPos.x + conf.frame_size.x, window->DC.CursorPos.y + conf.frame_size.y));
    ...
    if (conf.values.count > 0) {
        ...
        if (conf.grid_x.show) {
            ...
            case PlotConfig::Scale::Linear: {
                ...
                float inc = 1.f / cnt;
                for (int i = 0; i <= cnt; ++i) {
                    float x0 = static_cast<float>(ImLerp(inner_bb.Min.x, inner_bb.Max.x, i * inc));
            ...
            case PlotConfig::Scale::Log10: {
                ...
                while (start < x_max) {
                    for (int i = 1; i < 10; ++i) {
                        ...
                        float x0 = static_cast<float>(ImLerp(inner_bb.Min.x, inner_bb.Max.x, t));
                        window->DrawList->AddLine(
                            ImVec2(x0, y0),
                            ImVec2(x0, y1),
        ...
        if (conf.selection.show) {
            if (hovered) {
                if (g.IO.MouseClicked[0]) {
                    ...
                    //static cast as there is a warning if they don't match signed or unsigned
                    if (end < static_cast<unsigned>(conf.values.offset + conf.values.count)) {
